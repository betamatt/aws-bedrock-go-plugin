...
func TestStripInferenceProfilePrefix(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "global prefix",
			input:    "global.anthropic.claude-3-haiku-20240307-v1:0",
			expected: "anthropic.claude-3-haiku-20240307-v1:0",
		},
		{
			name:     "us prefix",
		...
		{
			name:     "eu prefix",
			input:    "eu.anthropic.claude-3-opus-20240229-v1:0",
			expected: "anthropic.claude-3-opus-20240229-v1:0",
		},
		{
			name:     "jp prefix",
			input:    "jp.amazon.nova-pro-v1:0",
			expected: "amazon.nova-pro-v1:0",
		},
		{
			name:     "apac prefix",
			input:    "apac.meta.llama3-8b-instruct-v1:0",
			expected: "meta.llama3-8b-instruct-v1:0",
		},
		{
			name:     "au prefix",
			input:    "au.mistral.mistral-large-2407-v1:0",
			expected: "mistral.mistral-large-2407-v1:0",
		},
		{
			name:     "us-gov prefix",
			input:    "us-gov.anthropic.claude-3-haiku-20240307-v1:0",
			expected: "anthropic.claude-3-haiku-20240307-v1:0",
		},
		{
			name:     "empty string",
		...
		{
			name:     "prefix-like but not a valid prefix",
			input:    "usa.anthropic.claude-3-haiku-20240307-v1:0",
			expected: "usa.anthropic.claude-3-haiku-20240307-v1:0",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := stripInferenceProfilePrefix(tt.input)
			if result != tt.expected {
				t.Errorf("stripInferenceProfilePrefix(%q) = %q, want %q", tt.input, result, tt.expected)
			}
		})
	}
}
